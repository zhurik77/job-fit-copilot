...
npm run shots    # перегенерировать скриншоты панели и широкие кадры
npm run site     # собрать лендинг из site/ в docs/ (GitHub Pages)
```

Тесты запускают `sidepanel/panel.js` вне Chrome через заглушки в
`tools/panel-harness.js`, поэтому чистые функции проверяются без браузера.

**Структура репозитория.** Само расширение не имеет зависимостей — весь
фронтенд-тулчейн живёт отдельно:

| Каталог | Что это |
|---|---|
| `sidepanel/`, `options/`, `content-scripts/`, `shared/`, `background.js` | Расширение. Ни одной runtime-зависимости |
| `tools/` | Скрипты скриншотов и заглушки для тестов |
| `test/` | `node --test`, без браузера |
| `site/` | Лендинг: Vite + React + Tailwind. Свой `package.json` |
| `docs/` | **Результат сборки** `site/`. Отдаётся GitHub Pages, правится только через `npm run site` |
| `launch/` | Материалы для Chrome Web Store и запуска |
