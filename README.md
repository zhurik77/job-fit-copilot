...
Большая часть проверок автоматизирована — см. `.github/workflows/ci.yml`:

- [x] Версия в `manifest.json` и `package.json` совпадает — **проверяет CI**.
- [x] README упоминает актуальную версию — **проверяет CI**.
- [x] `manifest.json` валиден, MV3, все указанные в нём файлы существуют — **проверяет CI**.
- [x] Lint и тесты проходят — **проверяет CI и блокирует релиз при провале**.
- [x] Тег совпадает с версией манифеста — **проверяет release-workflow**.
- [ ] Скриншоты в `docs/screenshot-*.jpg` — реальные захваты текущей вёрстки
      (см. `npm run shots`), не сгенерированы AI и не устарели визуально.
